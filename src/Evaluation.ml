...
(** retorna uma nova localização livre na memória *)
...
(** retorna uma memória com o valor `v` armazenado na localização `loc` *)
...
let rec step    (e     :    term)
    ...
    match e with
            ...
            | Integer _ | Boolean _ | Unit | Location _ ->
                Error ("Forma normal, termo " ^ ast_of_term e ^ " é um valor")
        ...
        | BinaryOperation (op, e1, e2) ->
          ...
          let rec multi_step (e : term) (mem : memory) (limit : int) : (term * memory * evaluation, string) result =
            ...
            (** se um termo é valor, retorne (termo, memória, regra termo -> valor) *)
            else if is_value_term e then (match typeinfer e [] with
                | Int -> Ok (e, mem, {}
